//
//  CurrencyConverterUITests.swift
//  CurrencyConverterUITests
//
//  Created by Dao Anh Phuc on 29/10/24.
//

import XCTest

final class CurrencyConverterUITests: XCTestCase {
    
    override func setUpWithError() throws {
        // Put setup code here. This method is called before the invocation of each test method in the class.
        
        // In UI tests it is usually best to stop immediately when a failure occurs.
        continueAfterFailure = false
        
        // In UI tests it’s important to set the initial state - such as interface orientation - required for your tests before they run. The setUp method is a good place to do this.
    }
    
    override func tearDownWithError() throws {
        // Put teardown code here. This method is called after the invocation of each test method in the class.
    }
    
    @MainActor
    func testExample() throws {
        // UI tests must launch the application that they test.
        let app = XCUIApplication()
        app.launch()
        
        // Use XCTAssert and related functions to verify your tests produce the correct results.
    }
    
    @MainActor
    func testLaunchPerformance() throws {
        if #available(macOS 10.15, iOS 13.0, tvOS 13.0, watchOS 7.0, *) {
            // This measures how long it takes to launch your application.
            measure(metrics: [XCTApplicationLaunchMetric()]) {
                XCUIApplication().launch()
            }
        }
    }
    
    @MainActor
    func testCurrencyConversion() throws {
        let app = XCUIApplication()
        app.launch()

        sleep(2)
        
        // Ensure the app is in the correct initial state
        XCTAssertTrue(app.navigationBars["Currency Converter"].exists)
        
        // Enter an amount
        let amountTextField = app.textFields["Enter an amount"]
        XCTAssertTrue(amountTextField.exists)
        amountTextField.tap()
        amountTextField.typeText("100")
        
        // Select base currency
        let fromPicker = app.pickers["From:"]
        XCTAssertTrue(fromPicker.exists)
        fromPicker.tap()
        app.pickerWheels.element.adjust(toPickerWheelValue: "EUR")
        
        // Select target currency
        let toPicker = app.pickers["To:"]
        XCTAssertTrue(toPicker.exists)
        toPicker.tap()
        app.pickerWheels.element.adjust(toPickerWheelValue: "USD")
        
        // Tap the Convert button
        let convertButton = app.buttons["Convert"]
        XCTAssertTrue(convertButton.exists)
        convertButton.tap()
        
        // Verify the conversion result
        let resultText = app.staticTexts["120.00 USD"]
        XCTAssertTrue(resultText.exists)
    }
}
